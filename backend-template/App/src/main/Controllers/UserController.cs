...
public class UserController : ApiHelper
{
    ...
    public async Task<IActionResult> Login([FromBody] LoginRequestBody user)
    {
        ...
        // ✅ Token generieren
        string jwtToken = _authService.GenerateToken(existingUser, user.keepLoggedIn);

        // ✅ Token als Cookie setzen mit den richtigen Flags
        Response.Cookies.Append("token", jwtToken, new CookieOptions
        {
            HttpOnly = true,
            Secure = true, // ⬅️ MUSS für SameSite=None gesetzt sein
            SameSite = SameSiteMode.None, // ⬅️ erlaubt Cross-Origin Cookies
            Expires = user.keepLoggedIn ? DateTime.UtcNow.AddDays(30) : DateTime.UtcNow.AddDays(1)
        });
